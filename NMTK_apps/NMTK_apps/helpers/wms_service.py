...
def handleWMSRequest(request, datafile):
    ...
    style_field=request.GET.get('style_field', datafile.result_field or 'nmtk_id')
    ...
    if style_field not in attributes:
    ...
    elif field_attributes['type'] not in ('text',):
    ...
    try:
        ...
        mapfile_path=os.path.join(datafile.mapfile_path, "wms_{0}_ramp_{1}_{2}.map".format(datafile.pk,
                                                                                           ramp_id,
