...
def handleWMSRequest(request, datafile):
    ...
    if datafile.geom_type != 99:  # 99 is raster
        ...
        legend = LegendGenerator(description='Raster Overlay')
    ...
    try:
        if datafile.geom_type == 99:
            base_name = 'raster_wms_'
            mapfile_path = os.path.join(
                datafile.mapfile_path, "{0}{1}.map".format(
                    base_name, datafile.pk))
        else:
