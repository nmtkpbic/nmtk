...
#             ss=(max_value-min_value)/steps
    ...
    def __iter__(self):
        ...
        if (self.numeric and
            ...
            logger.debug(
                'Using min/max values: %s-%s', self.min_value, self.max_value)
            self.value_iterator = enumerate(
                np.linspace(self.min_value, self.max_value, num=self.steps))
        elif self.values_list:
            logger.debug('Using a values list: %s', self.values_list)
        ...
        logger.debug(
            'Returning now that iterator is ready %s', self.value_iterator)
        return self
    ...
    def generateLegendGraphic(self, width=257, component_height=16,
        ...
        if ((self.min_text is not None and self.max_text is not None)
            ...
            while total_text_width > width - border * 2 and font_size > 0:
                ...
                min_text_width, min_text_height = font.getsize(
                    '{0}'.format(self.min_text))
                max_text_width, max_text_height = font.getsize(
                    '{0}'.format(self.max_text))
                total_text_width = min_text_width + \
                    max_text_width + legend_text_min_separation
        ...
        if not im or self.other_features_color:
            logger.debug(
                'Generating enumerated (or other feature color) graphic')
            # In this case there wasn't a ramp, or there was a ramp and
            # other features existed, so we need to provide legend data for
            # that
            for color in self:
                ...
                if color['type'] in ('other', 'values'):
                    ...
                    # Now we can use value and color['rgba'] to produce the
                    # image and text.
                    color_legend = Image.new(
                        'RGBA', (width, component_height), "white")
                    draw = ImageDraw.Draw(color_legend)
                    legend_color = "rgba({0},{1},{2},{3})".format(
                        *color['rgba'])
                    for i in range(0, component_height):
                        draw.line(
                            (i, 0, i, component_height), fill=legend_color)
                    # A little fudge here - some glyphys don't fit in the drawing area due to their script/style,
                    # which causes some of the glyphs to go outside the bounds.  So we add
                    # three pixels to the height to compensate - which works for the "default"
