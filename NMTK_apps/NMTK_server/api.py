...
class CSRFBypassSessionAuthentication(SessionAuthentication):
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        return super(
            CSRFBypassSessionAuthentication,
...
class UserResourceValidation(Validation):
    ...
    def is_valid(self, bundle, request=None):
        ...
        if bundle.request.method in ('PUT', 'PATCH'):
            if ('password' in bundle.data and not bundle.request.user.is_superuser):
                if 'current_password' not in bundle.data:
                    errors[
                        '__all__'] = 'current_password must be supplied when changing password'
...
class UserResourceAuthorization(Authorization):
    ...
    def create_detail(self, object_list, bundle):
        ...
        if not bundle.request.user.is_superuser:
            raise Unauthorized(
                'You lack the privilege to create this resource')
        return True
    ...
    def update_list(self, object_list, bundle):
        ...
        if bundle.request.user.is_superuser:
            return object_list
        raise Unauthorized(
            'You lack the privilege to update resources this way')
...
class DataFileResource(ModelResource):
    ...
    field_attributes = fields.CharField(
        'field_attributes', readonly=True, null=True)
    result_field = fields.CharField('result_field', readonly=True, null=True)
    result_field_units = fields.CharField(
        'result_field_units', readonly=True, null=True)
...
#             raise Http404
    ...
    def download_detail(self, request, **kwargs):
        ...
        if allow_download:
            ...
            raise Unauthorized(
                'You lack the privileges required to download this file')
    ...
    def dehydrate(self, bundle):
        ...
        '''
        if bundle.obj.file:
            bundle.data['file'] = reverse(
                "api_%s_download_detail_file" %
                (self._meta.resource_name,
                 ),
                kwargs={
                    'resource_name': self._meta.resource_name,
        ...
        if bundle.obj.processed_file:
            bundle.data['download_url'] = reverse(
                "api_%s_download_detail" %
                (self._meta.resource_name,
                 ),
                kwargs={
                    'resource_name': self._meta.resource_name,
...
class MapColorStyleResource(ModelResource):
    ...
    def dehydrate(self, bundle):
        ...
        if bundle.obj.ramp_graphic:
            bundle.data['ramp_graphic'] = reverse(
                "api_%s_download_ramp_graphic" %
                (self._meta.resource_name,
                 ),
                kwargs={
                    'resource_name': self._meta.resource_name,
    ...
    def prepend_urls(self):
        return [
            url(
                r"^(?P<resource_name>%s)/(?P<pk>\w[\w/-]*)/ramp_graphic.png$" %
                (self._meta.resource_name,
                 ),
                self.wrap_view('download_ramp_graphic'),
                name="api_%s_download_ramp_graphic" %
                (self._meta.resource_name,
                 )),
        ]
...
class ToolSampleFileResource(ModelResource):
    ...
    def dehydrate(self, bundle):
        ...
        if bundle.request.user.is_authenticated():
            bundle.data['load_sample_data'] = reverse(
                "api_%s_load_sample_data" %
                (self._meta.resource_name,
                 ),
                kwargs={
                    'resource_name': self._meta.resource_name,
            ...
            if bundle.obj.pk:
                for field in ['tool', ]:
                    if (getattr(bundle.obj, '_old_{0}'.format(field), None) !=
                        ...
                        logger.debug(
                            'Old value was: %s', getattr(
                                bundle.obj, '_old_{0}'.format(field)))
                        errors[field] = ('This field cannot be changed, ' +
                                         'please create a new job instead')
            if bundle.obj.status != 'U':
                errors['status'] = (
                    'Cannot update a job once it has been ' +
                    'configured, please create a new job instead (%s)' %
                    (bundle.obj.status))
                ...
                if validator.is_valid():
                    ...
                    # Returns a valid tool config.
                    bundle.obj.config = validator.genToolConfig(force=True)
                    # Stick the job files here, we can have the model save them.
                    # the model looks at job_files_pending when it sees a state change,
                    # and only then will it save the job files.
                    bundle.obj.job_files_pending = validator.genJobFiles(
...
#                 bundle.data['job_files'].append(f.json())
        ...
        try:
            ...
            bundle.data['last_status'] = bundle.obj.jobstatus_set.all()[
                0].timestamp
        except IndexError:
            pass
        return bundle
