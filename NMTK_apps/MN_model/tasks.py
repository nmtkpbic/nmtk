...
def performModel(data_file, 
    ...
    job_config_loaded=json.loads(open(job_config).read()).get('analysis settings',{})
    property_map={}
    parameters={}
    failures=[]
    if (not isinstance(job_config_loaded, (dict))):
       failures.append('Please provide analysis settings')
    else:
       logger.debug("Loaded config: %s", job_config_loaded)
